...
import { fetchAdjustmentReasons } from '../../services/settingsService';
...
const AdjustmentReasons: React.FC = () => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      fetchAdjustmentReasons().then(setReasons).catch(console.error);
    }
  }, [isAuthenticated]);
  ...
  const handleAddReason = async () => {
    ...
    try {
      ...
      setSuccess('Adjustment reason added successfully!');
      setTimeout(() => setSuccess(''), 3000); // Auto-hide success message
    } catch (err) {
      setError('Failed to add adjustment reason');
    }
  };
  ...
  return (
    <Box sx={{ padding: 3, maxWidth: '1200px', margin: '0 auto', bgcolor: 'white', borderRadius: 2, boxShadow: '0 0 15px rgba(0,0,0,0.1)' }}>
      <Typography variant="h5" sx={{ mb: 3, color: '#00BCD4' }}>Adjustment Reasons</Typography>
