...
const StockStatusReport: React.FC = () => {
  ...
  const renderSection = (items: Medicine[], title: string) => (
    <section style={{ marginBottom: 24, background: theme.palette.background.paper, border: `1px solid ${theme.palette.divider}`, borderRadius: 16, boxShadow: theme.shadows[1] }}>
              ...
              {items.length > 0 ? (
                items.map((medicine, index) => (
                  <tr key={`${medicine.product_name}-${index}`} style={{ transition: 'background-color 0.15s ease-in-out', background: index % 2 === 0 ? theme.palette.background.paper : theme.palette.action.hover }}>
                    ...
                    <td style={{ padding: '16px 20px', color: theme.palette.text.primary }}>{medicine.expiry_date}</td>
    ...
    <div style={{ minHeight: '100vh', background: theme.palette.background.default, padding: 32 }}>
      <header style={{ background: theme.palette.background.paper, boxShadow: theme.shadows[1], marginBottom: 24 }}>
        <div style={{ maxWidth: '100%', padding: '16px 24px' }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: theme.palette.text.primary }}>Store Stock Status Report</h1>
      </div>
      </header>
      ...
      {/* Filters */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <label style={{ fontSize: 14, fontWeight: 500, color: theme.palette.text.primary }}>Search Medicines:</label>
            <div style={{ position: 'relative' }}>
              <div style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: theme.palette.text.secondary }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <label style={{ fontSize: 14, fontWeight: 500, color: theme.palette.text.primary }}>Filter by Section:</label>
          <select
              style={{
                padding: '12px 12px',
                border: `1px solid ${theme.palette.divider}`,
      ...
      {loading && (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '40px 0' }}>
            <Spinner className="h-8 w-8" style={{ color: theme.palette.primary.main }} />
        </div>
      )}
      {error && (
          <div style={{ textAlign: 'center', color: theme.palette.error.main, background: theme.palette.error.light, padding: '16px 24px', borderRadius: 8, boxShadow: theme.shadows[1] }}>
          {error}
        </div>
      )}
