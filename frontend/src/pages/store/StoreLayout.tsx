...
const StoreManagementLayout: React.FC = () => {
  ...
  const navItems = [
    { label: 'Items Manager', path: '/store/ItemsManagers', icon: <Inventory2Icon /> },
    { label: 'Stock Taking', path: '/store/StockTaking', icon: <AssignmentTurnedInIcon /> },
    { label: 'Sock Taking Report', path: '/store/StockTakingReport', icon: <SummarizeIcon /> },
    { label: 'Genelar Stock Report', path: '/store/StoreReport', icon: <AssessmentIcon /> },
  ];
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: theme.palette.background.default }}>
      ...
      <Box sx={{ display: 'flex', flexGrow: 1 }}>
        ...
        >
          <Typography variant="h6" sx={{ mb: 2, color: '#FF5722', fontWeight: 600 }}>Store Menu</Typography>
          <Divider sx={{ mb: 2 }} />
          {renderNavItems()}
        </Drawer>
