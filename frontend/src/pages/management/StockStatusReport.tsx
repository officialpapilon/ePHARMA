...
const StockStatusReport: React.FC = () => {
  ...
  const debouncedFetch = useCallback(
    debounce((...args: unknown[]) => {
      // Ensure the arguments are numbers before calling fetchMedicines
      const [page, perPage] = args;
      fetchMedicines(Number(page), Number(perPage));
    }, 500),
    [searchTerm]
  );
  ...
  const exportToPDF = (data: Medicine[], title: string) => {
    ...
    // Use the correct way to get the finalY from autoTable
    
    const summaryY = (doc as any).lastAutoTable?.finalY ? (doc as any).lastAutoTable.finalY + 10 : 54;
    doc.setFontSize(12);
