...
export const fetchPharmacySettings = async (): Promise<PharmacySetting[]> => {
  const response = await fetch('http://127.0.0.1:8000/api/settings/pharmacy');
  if (!response.ok) throw new Error('Failed to fetch pharmacy settings');
  return response.json();
};

export const fetchPaymentMethods = async (): Promise<PaymentMethod[]> => {
  const response = await fetch('http://127.0.0.1:8000/api/settings/payment-methods');
  if (!response.ok) throw new Error('Failed to fetch payment methods');
  return response.json();
};

export const fetchStockTakingReasons = async (): Promise<Reason[]> => {
  const response = await fetch('http://127.0.0.1:8000/api/settings/stock-taking-reasons');
...
export const fetchAdjustmentReasons = async (): Promise<Reason[]> => {
  const response = await fetch('http://127.0.0.1:8000/api/settings/adjustment-reasons');
...
export const fetchExpenseCategories = async (): Promise<ExpenseCategory[]> => {
  const response = await fetch('http://127.0.0.1:8000/api/settings/expense-categories');
  if (!response.ok) throw new Error('Failed to fetch expense categories');
  return response.json();
};

export const fetchUsers = async (): Promise<User[]> => {
  const response = await fetch('http://127.0.0.1:8000/api/settings/users');
  if (!response.ok) throw new Error('Failed to fetch users');
  return response.json();
};

export const fetchSystemSettings = async (): Promise<SystemSetting[]> => {
  const response = await fetch('http://127.0.0.1:8000/api/settings/system');
